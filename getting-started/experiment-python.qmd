...
---
We use the [`coco-experiment` (`cocoex`)](https://pypi.org/project/coco-experiment) Python module to benchmark an algorithm.
...
For running the benchmarking in several batches a somewhat expanded example `example_experiment_complete.py` can be found in [this subfolder](https://github.com/numbbo/coco/blob/development/code-experiments/build/python/example).

#### Benchmarking "your own" algorithm
For benchmarking another algorithm,

1. copy-paste the above code or (preferably) download the linked batching example `benchmark_experiment_complete.py` and the `mkl_bugfix.py` file
...
4. execute the file from a system shell like 
   ```python
   python benchmark_experiment.py 1 # under *nix, consider using "nohup nice ..."
   ```
   or from an ipython shell or notebook like
   ```python
   %run benchmark_experiment.py 1
   ```
