...
### go
while tracker.remaining_problems(len(suite)):
    for problem in suite:  # this loop will take 2-3 minutes
        if tracker.skip(problem):
            continue
...
For benchmarking another algorithm, copy-paste the above code, reassign `fmin` respectively and adapt its calling code `fmin(problem, ...)`.
