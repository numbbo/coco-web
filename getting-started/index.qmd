...
and is divided into two main parts which can be used independently: [`coco-experiment`](#running-an-algorithm-on-a-coco-benchmark-suite-experiment) for running experiments and [`coco-postprocess`](#displaying-benchmarking-results-postprocess) for displaying benchmarking data.
...
The last line postprocesses the obtained data and compares the result with BFGS (see also below [Displaying Benchmarking Results (postprocess)](displaying-benchmarking-results-postprocess)).
...
5. inspect the figures (and tables) generated by `cocopp.main`, see also [Displaying Benchmarking Results (postprocess)](displaying-benchmarking-results-postprocess) below.
...
```
gives, for example, these runtime profiles (which are, broadly speaking, empirical runtime distributions, see @hansen2022anytime) for three algorithms for all [`bbob`-functions](testbeds/bbob) (left) and for `bbob`-functions 6 and 14 in 20-D.
